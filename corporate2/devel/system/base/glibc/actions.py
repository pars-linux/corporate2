...
defaultflags = "-O3 -fomit-frame-pointer -mno-tls-direct-seg-refs -g -ggdb"
...
config = {"multiarch": {
                "multi": True,
                "extraconfig": "--build=i686-pc-linux-gnu --enable-multi-arch --libdir=/usr/lib32",
                "coreflags":   "-m32",
                "libdir":      "lib32",
                "buildflags":  "-mtune=generic -march=i686 -O3 -pipe -fomit-frame-pointer %s" % defaultflags,
    ...
    for root, dirs, files in os.walk(_dir):
        for name in files:
            ...
            if ("crt" in name and name.endswith(".o")) or name.endswith(".a"):
                ...
                shelltools.system('objcopy -R ".note.gnu.build-id" %s' % i)
