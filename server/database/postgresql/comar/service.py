...
def check_postgresql():
    import os
    if not os.path.exists(config.get("PGDATA", "/var/lib/pgsql/data")):
        fail(MSG_ERR_PGSQLNOTINST)

PIDFILE = "%s/postmaster.pid" % config.get("PGDATA", "/var/lib/pgsql/data")

@synchronized
def start():
    check_postgresql()
    startService(command="/usr/bin/pg_ctl",
                 args=["start", "-D", config.get("PGDATA", "/var/lib/pgsql/data"), "-l", config.get("PGLOG", "/var/lib/pgsql/data/postgresql.log"), "-o", config.get("PGOPTS", "")],
                 pidfile=PIDFILE,
...
def stop():
    stopService(command="/usr/bin/pg_ctl",
                args=["stop", "-D", config.get("PGDATA", "/var/lib/pgsql/data"), "-s", "-m", "fast"],
                chuid=config.get("PGUSER", "postgres"),
                donotify=True)

def reload():
    stopService(command="/usr/bin/pg_ctl",
                args=["reload", "-D", config.get("PGDATA", "/var/lib/pgsql/data"), "-s"],
                donotify=True)
